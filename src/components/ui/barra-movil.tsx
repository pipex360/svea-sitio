 ...
 *   Cotizar. Se retira mientras el formulario de contacto está a la vista,
 *   para no taparlo.
...
export function BarraMovil({ base = '', copia = false }: { base?: string; copia?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (!contacto) return;
    const observador = new IntersectionObserver(([e]) => setFormularioALaVista(e.isIntersecting), {
      threshold: 0.25,
    });
    ...
    <>
      ...
      <div
        className={cn(
          ...
          formularioALaVista ? 'translate-y-[120%]' : 'translate-y-0',
        )}
        style={{ bottom: abajo }}
        aria-hidden={formularioALaVista}
        inert={formularioALaVista || undefined}
      >
        <a
          href={WHATSAPP}
          target="_blank"
